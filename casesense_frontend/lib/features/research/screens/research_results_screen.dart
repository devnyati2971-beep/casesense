...
class ResearchResultsScreen extends ConsumerWidget {
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('Research Results', style: Theme.of(context).textTheme.displayMedium),
                  ...
                  if (researchState.isLoading)
                    ...
                    _EmptyResults(sessionId: sessionId)
                  else
                    for (final group in groups) ...[
                      ...
                      for (final judgment in (group['judgments'] as List<dynamic>? ?? []))
                        Padding(
...
class _ResultCardWithActions extends ConsumerStatefulWidget {
  ...
  const _ResultCardWithActions({required this.judgmentData, required this.sessionId});
  @override
  ConsumerState<_ResultCardWithActions> createState() => _ResultCardWithActionsState();
}

class _ResultCardWithActionsState extends ConsumerState<_ResultCardWithActions> {
  bool _saving = false;
  bool _saved = false;
  String? _translated;
  bool _translating = false;

  bool get _isAuthed => ref.read(authControllerProvider).value?.isAuthenticated ?? false;
  ...
  Widget build(BuildContext context) {
    final Map<String, dynamic> judgment = widget.judgmentData is Map<String, dynamic>
        ? widget.judgmentData
        : <String, dynamic>{};
    final j = (judgment['judgment'] as Map<String, dynamic>?) ?? <String, dynamic>{};
    ...
    final proposition = propositions.isNotEmpty && propositions.first is Map<String, dynamic>
    ...
    final passage = passages.isNotEmpty && passages.first is Map<String, dynamic>
              ...
              child: Row(
                ...
                children: [
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            Text(court, style: Theme.of(context).textTheme.labelSmall),
                            const SizedBox(width: 12),
                            Text('•', style: TextStyle(color: AppColors.subtleBronze)),
                            const SizedBox(width: 12),
                            Text(relevance, style: Theme.of(context).textTheme.labelSmall?.copyWith(color: AppColors.antiqueBrass)),
                          ],
                        ),
                        const SizedBox(height: 8),
                        Text(caseName, style: Theme.of(context).textTheme.headlineMedium),
                        if (citation?.toString().isNotEmpty == true) ...[
                          const SizedBox(height: 4),
                          Text(citation.toString(), style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: AppColors.warmGrey)),
                        ],
                        if (explanation?.toString().isNotEmpty == true) ...[
                          const SizedBox(height: 8),
                          Text(explanation.toString(), style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: AppColors.subtleBronze, fontStyle: FontStyle.italic)),
                        ],
                      ],
                    ),
                  ),
                  if (sourceUrl?.toString().isNotEmpty == true)
                    PrimaryButton(
                      label: 'Read Judgment',
                      onPressed: () => context.push('/judgment/${j['id'] ?? '0'}'),
                    ),
                ],
              ),
            ),
                ...
                children: [
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('AI Proposition', style: Theme.of(context).textTheme.labelSmall),
                        const SizedBox(height: 8),
                        Text(propText, style: Theme.of(context).textTheme.bodyLarge),
                      ],
                    ),
                  ),
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            '“$passageText”',
                            style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: AppColors.ivory, fontStyle: FontStyle.italic),
                          ),
                          if (_translated != null) ...[
                            ...
                            Container(
                              ...
                              child: Column(
                                ...
                                children: [
                                  Row(
                                    children: [
                                      const Icon(Icons.g_translate, color: AppColors.antiqueBrass, size: 13),
                                      const SizedBox(width: 6),
                                      Text('हिन्दी अनुवाद', style: Theme.of(context).textTheme.labelSmall?.copyWith(color: AppColors.antiqueBrass)),
                                    ],
                                  ),
                                  const SizedBox(height: 8),
                                  Text(_translated!, style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: AppColors.ivory)),
                                ],
                              ),
                            ),
                          ],
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  OutlinedButton.icon(
                    onPressed: _saved || _saving ? null : () { if (_requireAuth()) _saveCitation(j, passage, propText); },
                    icon: Icon(_saved ? Icons.bookmark : Icons.bookmark_border, size: 16),
                    label: Text(_saved ? 'Saved ✓' : 'Save Citation'),
                    style: OutlinedButton.styleFrom(
                      foregroundColor: _saved ? AppColors.success : AppColors.charcoal,
                      side: BorderSide(color: _saved ? AppColors.success : AppColors.stone),
                    ),
                  ),
                  const SizedBox(width: 12),
                  // Translate to Hindi (authed only — AI quota is per user)
                  OutlinedButton.icon(
                    onPressed: _translating ? null : () { if (_requireAuth()) _translatePassage(passageText); },
                    icon: _translating
                  ...
                  if (propositionId != null)
                    TextButton.icon(
                      onPressed: () { if (_requireAuth()) _selectAuthority(propositionId, j); },
  ...
  Future<void> _saveCitation(
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(ok ? 'Citation saved to your collection.' : 'Could not save citation.')),
    );
  }

  Future<void> _translatePassage(String passageText) async {
    if (passageText.isEmpty) return;
    setState(() => _translating = true);
    try {
      final dio = ref.read(dioProvider);
      final res = await dio.post('/translate', data: {'text': passageText, 'target_language': 'hi'});
      final raw = res.data is Map<String, dynamic> ? res.data as Map<String, dynamic> : <String, dynamic>{};
      if (mounted) {
        setState(() {
          _translated = (raw['translated_text'] ?? raw['data']?['translated_text'] ?? '[HI] $passageText').toString();
          _translating = false;
        });
      }
    } catch (_) {
  ...
  Future<void> _selectAuthority(String propositionId, Map<String, dynamic> judgment) async {
    final ok = await ref.read(authoritiesControllerProvider.notifier).selectAuthority(
      widget.sessionId,
      propositionId,
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(ok ? 'Authority selected for this session.' : 'Failed to select authority.')),
    );
  }
}
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          Text('No results yet', style: Theme.of(context).textTheme.headlineMedium),
          const SizedBox(height: 8),
          Text(
