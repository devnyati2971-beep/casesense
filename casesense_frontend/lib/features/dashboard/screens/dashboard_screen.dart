...
class _DashboardScreenState extends ConsumerState<DashboardScreen> {
  ...
  void initState() {
    super.initState();
    // Prefetch data for the dashboard sections (failures are non-fatal).
    // Guests skip authed sections entirely (v2.2 guest tier).
    // NOTE: ref.read is safe here because State is still mounted at the end of
    // initState/first build. Avoid reading ref *inside* addPostFrameCallback from
    // initState — that callback can fire after unmount on hot-reload / fast nav.
    final isAuthed = ref.read(authControllerProvider).value?.isAuthenticated ?? false;
    if (isAuthed) {
  ...
  void _submitSearch() {
    ...
    ref.read(researchControllerProvider.notifier).submitQuery(query).then((sessionId) {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    final fullName = (user['full_name'] as String?) ?? (isAuthed ? 'Advocate' : 'Guest');
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                          ...
                          Column(
                            ...
                            children: [
                              ...
                              Text(
                                ...
                                style: Theme.of(context).textTheme.bodyLarge?.copyWith(color: AppColors.charcoal, fontSize: 16),
                              ),
                            ],
                          ),
                            ...
                            Container(
                              ...
                              child: Column(
                                ...
                                children: [
                                  Row(
                                    children: [
                                      const Icon(Icons.auto_awesome, color: AppColors.antiqueBrass, size: 20),
                                      const SizedBox(width: 12),
                                      Text('AI-Powered Legal Assistant', style: Theme.of(context).textTheme.titleMedium),
                                    ],
                                  ),
                                        ...
                                        Expanded(
                                          child: TextField(
                                            ...
                                            decoration: InputDecoration(
                                              ...
                                              hintText: 'Tell CaseSense about your case...',
                                  ...
                                  const SizedBox(height: 16),
                                  Text(
                                    'You can also upload documents or use saved citations to get better results.',
                                    style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: AppColors.warmGrey, fontSize: 12),
                                  ),
                                  const SizedBox(height: 24),
                                  Row(
                                    children: [
                                      _FeatureBadge(icon: Icons.translate, title: 'Multi-language', subtitle: 'English / हिंदी / Bilingual'),
                                      const SizedBox(width: 24),
                                      Container(height: 30, width: 1, color: AppColors.stone),
                                      const SizedBox(width: 24),
                                      _FeatureBadge(icon: Icons.verified_user_outlined, title: 'Court-ready', subtitle: 'With relevant citations'),
                                      const SizedBox(width: 24),
                                      Container(height: 30, width: 1, color: AppColors.stone),
                                      const SizedBox(width: 24),
                                      _FeatureBadge(icon: Icons.lock_outline, title: 'Secure & Private', subtitle: 'Your data stays yours'),
                                    ],
                                  ),
                                ],
                              ),
                      ...
                      Container(
                        padding: const EdgeInsets.only(left: 56.0, right: 56.0, bottom: 40.0),
                        color: AppColors.parchment,
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                      ...
                      const SizedBox(height: 8),
                      Text('Jump into what you need to do.', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: AppColors.warmGrey)),
                            ...
                            child: isAuthed
                                ? Column(
                                    children: [
                                      ...
                                      const SizedBox(height: 24),
                                      _SavedCitationsPreview(count: savedState.citations.length),
                                      const SizedBox(height: 24),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(Icons.workspace_premium, color: AppColors.antiqueBrass, size: 22),
              const SizedBox(width: 10),
              Text('Unlock the full workspace', style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontSize: 20)),
            ],
          ),
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(label, style: Theme.of(context).textTheme.titleMedium?.copyWith(color: AppColors.ivory, fontSize: 13)),
        ],
      ),
    );
  }
      ...
      children: [
        ...
        Column(
          ...
          children: [
            ...
            Text(subtitle, style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: AppColors.warmGrey, fontSize: 11)),
          ],
        ),
      ],
    );
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(subtitle, style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: AppColors.subtleBronze, fontSize: 12)),
                  ],
                ),
              ),
              const Icon(Icons.arrow_forward, color: AppColors.charcoal, size: 18),
            ],
          ),
        ),
      ),
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('Recent Drafts', style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontSize: 20)),
              InkWell(
                onTap: () => context.push('/drafts_list'),
                child: Text('View All →', style: Theme.of(context).textTheme.titleMedium?.copyWith(color: AppColors.subtleBronze, fontSize: 13)),
              ),
            ],
          ),
          const SizedBox(height: 16),
          if (mattersState.matters.isEmpty)
            ...
            ...mattersState.matters.take(4).map((matter) => Padding(
              ...
              child: InkWell(
                ...
                child: Row(
                  children: [
                    Container(
                      padding: const EdgeInsets.all(8),
                      decoration: BoxDecoration(color: AppColors.charcoal, borderRadius: BorderRadius.circular(6)),
                      child: const Icon(Icons.description, color: AppColors.ivory, size: 16),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(matter['title'] ?? 'Untitled', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontSize: 14), overflow: TextOverflow.ellipsis),
                          Text(
                            '${matter['matter_type'] ?? 'matter'} • ${matter['court_name'] ?? '—'}',
                            style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: AppColors.subtleBronze, fontSize: 11),
                            overflow: TextOverflow.ellipsis,
                          ),
                        ],
                      ),
                    ),
                    const Icon(Icons.chevron_right, color: AppColors.warmGrey, size: 18),
                  ],
                ),
              ),
            )),
...
class _RecentSearchesSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Recent Searches', style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontSize: 20)),
          const SizedBox(height: 16),
          _SearchItem(context, 'bail under section 439'),
          _SearchItem(context, 'Article 21 speedy trial'),
          _SearchItem(context, 'basic structure doctrine'),
          const SizedBox(height: 8),
          InkWell(
            onTap: () => context.push('/finder'),
            child: Text('Open Citation Finder →', style: Theme.of(context).textTheme.titleMedium?.copyWith(color: AppColors.subtleBronze, fontSize: 13)),
          ),
        ],
      ),
    );
  ...
  Widget _SearchItem(BuildContext context, String query) {
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 10),
          Expanded(child: Text(query, style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontSize: 13))),
        ],
      ),
    );
  }
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('Saved Citations', style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontSize: 20)),
              Text('$count saved', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: AppColors.subtleBronze, fontSize: 12)),
            ],
          ),
          const SizedBox(height: 12),
          SizedBox(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(Icons.local_library, color: AppColors.antiqueBrass, size: 20),
              const SizedBox(width: 10),
              Text('Library', style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontSize: 20, color: AppColors.ivory)),
            ],
          ),
          const SizedBox(height: 8),
          Text(
