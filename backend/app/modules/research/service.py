...
class ResearchService:
    ...
    async def initiate_query_research(
        self, user_id: uuid.UUID, query: str, matter_id: Optional[uuid.UUID]
    ) -> Tuple[uuid.UUID, str]:
        ...
        if pool:
            await pool.enqueue_job("execute_research", str(session_id), _job_id=job_id, _queue_name="research")

        await AuditService.log(
            db=self.db, user_id=user_id, matter_id=matter_id, action="RESEARCH_START",
            resource_type="RESEARCH_SESSION", resource_id=str(session_id), detail={"mode": "QUERY"}
        )
    ...
    async def get_session_status(self, session_id: uuid.UUID, user_id: uuid.UUID) -> ResearchSession:
    ...
    async def list_sessions(self, user_id: uuid.UUID, limit: int = 50, cursor: Optional[str] = None) -> Tuple[List[ResearchSession], Optional[str]]:
        stmt = (
            select(ResearchSession)
            .where(ResearchSession.created_by == user_id)
