...
class ResearchSession(Base):
    ...
    # v2.2 guest flow: NULL for anonymous (guest) searches keyed by IP.
    created_by: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("users.id", ondelete="CASCADE"),
        nullable=True,
    )
    # IP (or other identity) that created a guest session; ownership check for polls.
    guest_key: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
    ...
    # Grouped research results ({groups: [...]}) — written by the research worker.
