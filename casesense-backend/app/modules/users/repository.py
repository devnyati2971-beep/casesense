...
from app.modules.users.models import AuthToken, EmailOutbox, RefreshToken, User
...
class UserRepository:
    ...
    async def get_refresh_token_including_revoked(self, raw_token: str) -> RefreshToken | None:
        """Look up a refresh token even when revoked (reuse detection, §14)."""
        token_hash = self._hash_token(raw_token)
        result = await self.session.execute(
    ...
    async def revoke_all_except_family(self, user_id: uuid.UUID, family_id: uuid.UUID) -> None:
        """Revoke every refresh token of the user except the given family (§14.4)."""
        await self.session.execute(
            update(RefreshToken)
            .where(
                RefreshToken.user_id == user_id,
                RefreshToken.revoked == False,
                (RefreshToken.family_id.is_(None)) | (RefreshToken.family_id != family_id),
            )
            .values(revoked=True, revoked_at=datetime.now(tz=timezone.utc))
        )

    async def revoke_family(self, family_id: uuid.UUID) -> None:
        """Revoke an entire refresh family (theft signal — §14)."""
        await self.session.execute(
            update(RefreshToken)
    ...
    # ── Auth tokens (verification / reset — v2.1) ─────────────────────────────
    ...
    async def store_auth_token(
        ...
        token = AuthToken(
            user_id=user_id,
            purpose=purpose,
            token_hash=self._hash_auth_token(raw_token),
            expires_at=expires_at,
    ...
    async def invalidate_auth_tokens(self, user_id: uuid.UUID, purpose: str) -> None:
        """Invalidate prior unexpired tokens of the same purpose (resend rule §14.2)."""
        await self.session.execute(
            update(AuthToken)
            .where(
                ...
                AuthToken.expires_at > datetime.now(tz=timezone.utc),
            )
            .values(used_at=datetime.now(tz=timezone.utc))
        )

    async def get_auth_token(self, purpose: str, raw_token: str) -> AuthToken | None:
        result = await self.session.execute(
            select(AuthToken).where(
                AuthToken.purpose == purpose,
                AuthToken.token_hash == self._hash_auth_token(raw_token),
            )
        )
        return result.scalar_one_or_none()
